---
title: "Model development"
output: html_notebook
---

```{r setup}
library(rstan)
library(cmdstanr)
library(tidyverse)
library(cowplot)
theme_set(theme_cowplot())

options(mc.cores = parallel::detectCores())
rstan_options(auto_write = TRUE)

source("R/evaluation_tools.R")
source("R/sbc.R")
source("R/sampling_multi.R")
source("R/hmm_tools.R")
source("R/hmm_devel_tools.R")

```


```{r}
#system.time({
model <- cmdstan_model(stan_file = here::here("hmm.stan"))
#})
#system.time({
#model <- rstan::stan_model(file = here::here("hmm.stan"))
#})
```



Single test

```{r}
adapt_delta <- 0.8
data <- sim_hmm(N_patients = 10, N_patients_unknown_load = 0, N_time = 10, N_treatments = 0, N_obs_per_patient = 4, N_ill_states = 1, prior = prior_hmm, use_severe_state = FALSE)
  #sim_hmm(N_patients = 20, N_patients_unknown_load = 20, N_time = 10, N_treatments = 0, N_obs_per_patient = 4, N_ill_states = 3, prior = prior_hmm, use_severe_state = FALSE)
if(inherits(model, "stanmodel")) {
  fit <- sampling(model, data = data$observed, control = list(adapt_delta = adapt_delta), chains = 1)
} else {
  cmdstan_fit <- model$sample(data = data$observed, adapt_delta = adapt_delta)
  fit <- rstan::read_stan_csv(cmdstan_fit$output_files())
}
evaluation_summary(fit, data$true)

plot_sim_data_true_hmm(data)
plot_sim_data_observed_hmm(data)

data$true


```

```{r}
pairs(fit)
```


```{r}
for(i in 1:5) {
  data <- sim_hmm(N_patients = 10, N_patients_unknown_load = 0, N_time = 10, N_treatments = 1, N_obs_per_patient = 4, N_ill_states = 3, prior = prior_hmm)
  
  (plot_sim_data_observed_hmm(data) + ggtitle("Observed",i)) %>% print()
  (plot_sim_data_true_hmm(data) + ggtitle("true",i)) %>% print()
  cat("\n======== ",i, " ===========\n")
  print(data$true)
}
```


SBC

```{r}
# cache_dir <- here::here("local_tmp")
# if(!dir.exists(cache_dir)) {
#   dir.create(cache_dir)
# }

adapt_delta <- 0.95
sbc_res <- sbc(model, generator = function() {sim_hmm(N_patients = 10, N_patients_unknown_load = 0, N_time = 10, N_treatments = 1, N_obs_per_patient = 4, N_ill_states = 1, prior = prior_hmm, use_severe_state = TRUE)}, N_steps = 50, control = list(adapt_delta = adapt_delta))

sbc_res$params %>% filter(grepl("beta|sigma", param_name)) %>% plot_sbc_params(binwidth = 10)
summarise_sbc_diagnostics(sbc_res)
sbc_res$diagnostics
```


```{r}
id <- 14
data <- list(observed = sbc_res$data[[id]], true = sbc_res$true_values[[id]])
if(inherits(model, "stanmodel")) {
  fit <- sampling(model, data = data$observed, control = list(adapt_delta = adapt_delta))
} else {
  cmdstan_fit <- model$sample(data = data$observed, adapt_delta = adapt_delta)
  fit <- rstan::read_stan_csv(cmdstan_fit$output_files())
}
evaluation_summary(fit, sbc_res$true_values[[id]])
data$true

plot_sim_data_observed(data) %>% print()

launch_shinystan_nonblocking(fit)


#sbc_res$true_values[[i]]
```

```{r}
plot_sim_data_observed(data)

```


```{r}
#bayesplot::mcmc_parcoord(fit, regex_pars = c("initial_disease_raw"), np = bayesplot::nuts_params(fit))
#bayesplot::mcmc_parcoord(fit, regex_pars = c("baseline_slopes_raw"), np = bayesplot::nuts_params(fit))

bayesplot::mcmc_parcoord(fit, pars = c("sigma","baseline_slopes_mean","baseline_slopes_sd"), np = bayesplot::nuts_params(fit))

bayesplot::mcmc_parcoord(fit, pars = c("sigma","baseline_slopes_mean","baseline_slopes_sd","t_high"), np = bayesplot::nuts_params(fit))
```
