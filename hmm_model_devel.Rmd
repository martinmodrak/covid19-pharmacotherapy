...
source("R/hmm_model_devel_tools.R")
...
#})
#system.time({
model <- rstan::stan_model(file = here::here("hmm_pcr.stan"))
#})
```
...
```{r}
adapt_delta <- 0.8
data <- simulator(N_patients = 5, N_time = 5, prior = prior)
data
...
```{r}
sbc_res <- sbc(model, generator = function() {simulator(N_patients = 5, N_time = 8, prior = prior)}, N_steps = 200, control = list(adapt_delta = adapt_delta))
