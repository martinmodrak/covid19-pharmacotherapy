...
wide_for_model <- gautret #%>% filter(Source == "Excluded") %>% mutate(NumericID = 1:n())
...
data_for_model <- c(prior_hmm, list(
        ...
        fixed_prior_sd = rep(1, 2 + use_time_effect),
...
X <- array(0, c(data_for_model$N_patients, data_for_model$N_time, 3))
