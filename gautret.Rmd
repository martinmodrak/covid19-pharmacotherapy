...
observation_type <- case_when(gautret_long$PCR == "NEG" ~ o_neg,
                              gautret_long$PCR == "POS" ~ o_pos,
                              gautret_long$PCR  %in% c("ICU","DEATH") ~ o_severe,
                              TRUE ~ o_pos
                                     )
...
data_for_model <- c(prior_hmm, list(
        ...
        times = gautret_long$Day + gautret_long$Days_From_Onset_Imputed + 1,
        o_types = observation_type,
        viral_load_known = !(gautret_long$PCR %in% c("POS","NEG","ICU","DEATH")),
        fixed_prior_sd = rep(1, 2) 
      ))

data_for_model$viral_load = rep(0, nrow(gautret_long))
data_for_model$viral_load[data_for_model$viral_load_known] <- 
  as.double(gautret_long$PCR[data_for_model$viral_load_known])
...
data_for_model <- c(data_for_model, transform_predictors_to_unique(X))
